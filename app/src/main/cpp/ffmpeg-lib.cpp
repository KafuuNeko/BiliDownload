...
inline auto MakeFormatPair() {
    ...
    return std::unique_ptr<FormatPair, decltype(deleter)>(new FormatPair(nullptr, nullptr),
                                                          deleter);
}
 ...
 * */
int32_t
VideoFormatConversion(const std::string &inputFile, const std::string &outputFile) noexcept {
...
int32_t ExtractAudio(const std::string &inputFile, const std::string &outputFile) noexcept {
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "extractAudio", "Completed audioCodecName=%s",
                        audioCodecName);
...
const char *GetVideoAudioFormat(const std::string &inputFile) {
    ...
    if (rc < 0) {
        __android_log_print(ANDROID_LOG_DEBUG, "getVideoAudioFormat",
                            "Failed to retrieve input stream information");
        return nullptr;
    }
...
#define JNI_FUNC(return_type, func_name) \
extern "C" \
JNIEXPORT return_type JNICALL \
Java_cc_kafuu_bilidownload_common_jniexport_JniTools_##func_name

JNI_FUNC(jstring, ffmpegInfo)(JNIEnv *env, jobject thiz) {
    return env->NewStringUTF(avcodec_configuration());
}

JNI_FUNC(jint, videoFormatConversion)(JNIEnv *env, jobject thiz, jstring in_file,
                                      jstring out_file) {
    return VideoFormatConversion(env->GetStringUTFChars(in_file, JNI_FALSE),
                                 env->GetStringUTFChars(out_file, JNI_FALSE));
}

JNI_FUNC(jint, extractAudio)(JNIEnv *env, jobject thiz, jstring in_file, jstring out_file) {
    return ExtractAudio(env->GetStringUTFChars(in_file, JNI_FALSE),
                        env->GetStringUTFChars(out_file, JNI_FALSE));
}

JNI_FUNC(jstring, getVideoAudioFormat)(JNIEnv *env, jobject thiz, jstring in_file) {
    return env->NewStringUTF(GetVideoAudioFormat(env->GetStringUTFChars(in_file, JNI_FALSE)));
}

JNI_FUNC(jstring, getMediaInfo)(JNIEnv *env, jobject thiz, jstring filename) {
